...
for company_symbol in company_symbols:
    ...
    # Remove missing or NaN values from the stock_data
    stock_data = stock_data.dropna()

    # Download historical price data for the market index (e.g., S&P 500)
    market_index_symbol = "MARKET_INDEX_SYMBOL"  # Replace with the actual market index symbol
    market_data = yf.download(market_index_symbol, start="2018-01-01", end="2022-12-31")

    # Calculate daily returns based on the closing prices of the market index
    market_returns = market_data["Close"].pct_change().dropna().values.reshape(-1, 1)
    stock_returns = stock_data["Return"].values.reshape(-1, 1)
    ...
    returns_sorted = np.sort(stock_data["Return"].values)
    ...
    with open(filename, "a") as file:
        ...
        if not file_exists:
            writer.writerow(["Company Name", "Stock Symbol", "Standard Deviation", "Beta", "Value at Risk", "P/E Ratio",
                             "Debt-to-Equity Ratio",
                             "Standard Deviation Label", "Beta Label", "Value at Risk Label"])
        writer.writerow(
            [stock_info["longName"], stock_symbol, std_dev, beta, var, pe_ratio, debt_to_equity, std_dev_label,
             beta_label, var_label])
